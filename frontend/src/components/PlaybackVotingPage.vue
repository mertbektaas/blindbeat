...
const emit = defineEmits(["intro-ready", "send-vote"]);
...
.track-column { display:grid; gap:.7rem; }.track-row { --track-accent:#c8f46b; display:grid; grid-template-columns:3.3rem minmax(9rem,1fr) minmax(8rem,1.8fr) 5.2rem; gap:1.1rem; align-items:center; min-height:5.4rem; padding:1rem 1.15rem; border:1px solid #35364d; color:var(--muted); background:#0b0c15; transition:.3s ease; }.track-row.active { border-color:var(--track-accent); color:var(--ink); background:color-mix(in srgb,var(--track-accent) 10%,#10111f); box-shadow:inset 4px 0 0 var(--track-accent); }.track-row.complete { opacity:.63; }.track-index,.track-state { font:.64rem "DM Mono",monospace; }.track-progress { height:4px; overflow:hidden; background:#2e2e43; }.track-progress i { display:block; height:100%; background:var(--track-accent); transition:width 80ms linear; }.track-state { justify-self:end; text-transform:uppercase; }
